...
namespace {
    ...
    void PerformanceGUI::actionPerformed(GObservable* source) {
        if (source == go) {
            ...
            /* If the dropdown selection is unavailable/invalid (e.g. UI interaction issues),
             * fall back to the one test we need for this assignment.
             */
            if (choice != kNotSelected && tests.containsKey(choice)) {
                startTests(*tests[choice]);
            } else if (tests.containsKey("fastMaxWeightMatching")) {
                startTests(*tests["fastMaxWeightMatching"]);
            }
        }
    }
