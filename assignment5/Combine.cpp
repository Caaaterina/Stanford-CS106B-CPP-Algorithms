...
/* Merges two sorted vectors of DataPoints by weight (O(n) on total length). */
static Vector<DataPoint> mergein2(const Vector<DataPoint>& left, const Vector<DataPoint>& right) {
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& toMerge) {
    ...
    if (toMerge.size() == 1) return toMerge[0];
    int mid = toMerge.size() / 2;
    Vector<Vector<DataPoint>> left = toMerge.subList(0, mid);
    Vector<Vector<DataPoint>> right = toMerge.subList(mid);
    Vector<DataPoint> a = combine(left);
    Vector<DataPoint> b = combine(right);
    return mergein2(a, b);
}
...
STUDENT_TEST("Combine with zero sequences returns empty.") {
    auto merged = combine({});
    EXPECT_EQUAL(merged.size(), 0);
}

STUDENT_TEST("One non-empty sequence and one empty sequence.") {
    DataPoint p1 = { "p1", 1.0 };
    DataPoint p2 = { "p2", 3.0 };
    auto merged = combine({ {}, { p1, p2 } });
    EXPECT_EQUAL(merged.size(), 2);
    EXPECT_EQUAL(merged[0], p1);
    EXPECT_EQUAL(merged[1], p2);

    merged = combine({ { p1, p2 }, {} });
    EXPECT_EQUAL(merged.size(), 2);
    EXPECT_EQUAL(merged[0], p1);
    EXPECT_EQUAL(merged[1], p2);
}

STUDENT_TEST("Multiple sequences all empty.") {
    auto merged = combine({ {}, {}, {} });
    EXPECT_EQUAL(merged.size(), 0);
}

STUDENT_TEST("Duplicate weights across sequences.") {
    DataPoint a = { "a", 10.0 };
    DataPoint b = { "b", 10.0 };
    DataPoint c = { "c", 20.0 };
    auto merged = combine({ { a }, { b }, { c } });
    EXPECT_EQUAL(merged.size(), 3);
    EXPECT_EQUAL(merged[2], c);
    EXPECT_EQUAL(merged[0].weight, 10.0);
    EXPECT_EQUAL(merged[1].weight, 10.0);
}

STUDENT_TEST("Unequal-length sequences.") {
    DataPoint d1 = { "d1", 1.0 };
    DataPoint d2 = { "d2", 2.0 };
    DataPoint d3 = { "d3", 3.0 };
    DataPoint d4 = { "d4", 4.0 };
    ...
    EXPECT_EQUAL(merged.size(), 5);
    EXPECT_EQUAL(merged[0], d1);
    EXPECT_EQUAL(merged[1], d2);
    EXPECT_EQUAL(merged[2], d3);
    EXPECT_EQUAL(merged[3], d4);
    EXPECT_EQUAL(merged[4], d5);
}
