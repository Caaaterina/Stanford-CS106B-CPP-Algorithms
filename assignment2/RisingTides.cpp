...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> flooded = Grid<bool>(terrain.numRows(), terrain.numCols(), false);
    Queue<GridLocation> q = {};
    
    for (GridLocation loc : sources) {
        if (terrain[loc.row][loc.col] <= height && !flooded[loc.row][loc.col]) {
            flooded[loc.row][loc.col] = true;
            q.enqueue(loc);
        }
    }
    ...
    while (!q.isEmpty()) {
        ...
        GridLocation up = {loc.row - 1, loc.col};
        GridLocation down = {loc.row + 1, loc.col};
        GridLocation left = {loc.row, loc.col -1};
        GridLocation right = {loc.row, loc.col + 1};
        Vector<GridLocation> neighbors = {up, down, left, right};
        for (GridLocation neigh : neighbors) {
            if (neigh.row < 0 || neigh.row >= terrain.numRows() || neigh.col < 0 || neigh.col >= terrain.numCols()) continue;

            if (flooded[neigh.row][neigh.col]) continue;

            if (terrain[neigh.row][neigh.col] <= height) {
                flooded[neigh.row][neigh.col] = true;
                q.enqueue(neigh);
            }
        }
    }
