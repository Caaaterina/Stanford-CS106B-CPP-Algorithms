...
bool overlapJudge(const Set<Shift>& sofar, const Shift& s) {
    /* TODO: Step 3
     * Return true iff shift s overlaps with any shift in sofar.
     * Hint: loop through sofar and call overlapsWith(cur, s).
     */
    ...
    for (const Shift& cur_s : sofar) {
        if (overlapsWith(cur_s, s)) {
            isOverlap = true;
            break;
        } // Remove this line once your overlap logic is in.
    }
...
Set<Shift> highestValueScheduleRec(const Set<Shift>& shifts, int maxHours, const Set<Shift>& sofar) {
     ...
     * If there are no shifts left OR no hours left, return sofar.
     */
    ...
    /* TODO: Step 2
     * Pick one shift to branch on.
     */
    Shift s = shifts.first();

    /* TODO: Step 4
     * Always compute the "exclude s" branch first.
     */
    auto without = highestValueScheduleRec(shifts - s, maxHours, sofar);
    int value1 = calculateValue(without);

    /* TODO: Step 5
     * If including s is illegal (overlap OR not enough hours), return without.
     */
    if (overlapJudge(sofar, s) || maxHours < lengthOf(s)) {
        ...
        return (value1 >= value2)? with : without;
    }

    /* TODO: Step 6
     * Compute the "include s" branch.
     */
    auto with = highestValueScheduleRec(
        shifts - s,
        maxHours /* replace with remaining hours */,
        sofar /* replace with updated set */
    );

    /* TODO: Step 7
     * Compare total values of with / without and return the better set.
     * Tie: either one is OK.
     */
    int valueWithout = 0; // replace with calculateValue(without)
    int valueWith = 0;    // replace with calculateValue(with)
    return (valueWithout >= valueWith) ? without : with;

}

Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    /* TODO: Step 0
     * Keep this function very small:
     * 1) If maxHours is negative, call error(...).
     * 2) Otherwise return the recursive helper with an empty chosen-set.
     */
    if (maxHours < 0) {
        error("TODO: write your error message");
    }
