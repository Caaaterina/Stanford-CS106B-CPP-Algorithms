...
static bool canBeMadeDisasterReadyrec(const Map<string, Set<string>>& roadNetwork,
                                      int numCitiesLeft,
...
static bool canBeMadeDisasterReadyrec(const Map<string, Set<string>>& roadNetwork,
    ...
    const string u = uncovered.first();
    Set<string> candidates = roadNetwork[u];
    candidates.add(u);

    for (const string& v : candidates) {
        Set<string> coveredByV;
        coveredByV.add(v);
        for (const string& n : roadNetwork[v]) {
            coveredByV.add(n);
        }
        Set<string> nextUncovered = uncovered;
        for (const string& x : coveredByV) {
            nextUncovered.remove(x);
        }
        supplyLocations.add(v);
        if (canBeMadeDisasterReadyrec(roadNetwork, numCitiesLeft - 1, supplyLocations, nextUncovered)) {
            return true;
        }
        supplyLocations.remove(v);
    }
}
