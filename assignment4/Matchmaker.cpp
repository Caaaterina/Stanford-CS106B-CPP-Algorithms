...
bool hasPerfectMatchingRec(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching,
                          Set<Pair>current, Set<string>unmatched){
    if (unmatched.isEmpty()) {
        matching = current;
        return true;
    }
    string person = unmatched.first();
    for (string pt: possibleLinks[person]) {
        if (unmatched.contains(pt)) {
            continue;
        }
        Set<string> nextUnmatched = unmatched;
        nextUnmatched.remove(person);
        nextUnmatched.remove(pt);
        Set<pair> nextCurrent = current;
        nextCurrent.add(Pair(person, pt));
        if (hasPerfectMatchingRec(possibleLinks, matching, nextCurrent, nextUnmatched)) {
            return true;
        }
    }
    return false;
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    for (stirng person: possibleLinks.keys()) {
        if (possibleLinks[person].isEmpty()) {
            return false;
        }
    }

    Set<string>unmatched;
    for (string person: possibleLinks.keys()) {
        unmatched.add(person);
    }
    Set<Pair>current;
    return hasPerfectMatchingRec(possibleLinks, unmatched, current, matching);
}

void maximumWeightMatchingRec(const Map<string, Map<string, int>>&possibleLinks, Set<string>remaining,
                                   int currentWeight, Set<Pair>currentEdegs, int& bestWeight, Set<Pair>&bestMatching){
    if (remaining.isEmpty()) {
        if (currentWeight > bestWeight) {
            bestWeight = currentWeight;
            bestMatching = currentEdges;
        }
        return;
    }

    string person = remaining.first();
    Set<string>nextRemaining = remaining;
    nextRemaining.remove(person);
    return maximumWeightMatchingRec(possibleLinks, nextRemaining, currentWeight, currentEdges, bestWeight, bestMatching);
    for (string pt: possibleLinks[person].keys()) {
        if (remaining.contains(pt)) {
            continue;
        }
        Set<string>nextRemaining = remaining;
        nextRemaining.remove(pt);
        nextRemaining.remove(person);
        Set<Pair>nextCurrentEdges = currentEdges;
        nextCurrentEdges.add(Pair(person, pt));
        int nextCurrentWeight = currentWeight + possibleLinks[person][pt];
        return maximumWeightMatchingRec(possibleLinks, nextRemaining, nextCurrentWeight,nextCurrentEdges, bestWeight, bestMatching);
    }
}
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    ...
    Set<Pair>currentEdges;
    int bestWeight = 0;
    for (string person: possibleLinks.keys()) {
        unmatched.add(person);
    }
    void maximumWeightMatchingRec(possibleLinks, remaining, 0,currentEdges, bestWeight, bestMatching){
        return bestMatching;
    }
}
...
STUDENT_TEST("Student: my tiny graph should have perfect matching") {
...
STUDENT_TEST("Student: output matching should be valid") {
    auto links = fromLinks({
        { "A", "B" },
        { "B", "C" },
        { "C", "D" },
...
STUDENT_TEST("Student: my tiny graph should have no perfect matching")  {
    auto links = fromLinks({
        { "A", "B" },
        { "B", "C" },
        { "C", "A" }
...
STUDENT_TEST("Student: max weight on two people") {
    auto links = fromWeightedLinks({
        { "A", "B", 5 }
    });
    EXPECT_EQUAL(maximumWeightMatching(links), Set<Pair>{ { "A", "B" } });
}
