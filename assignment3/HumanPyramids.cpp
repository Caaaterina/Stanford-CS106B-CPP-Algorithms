...
double weightOnBackRec(int row, int col, int pyramidHeight, Grid<double>& table) {
    ...
    if (row == 0 && col == 0) {
        ...
        double ans = 0;
        if (col > 0) {
            ans += 80 + weightOnBack(row - 1, col - 1, pyramidHeight, table) / 2;
        }
        if (col < row) {
            ans += 80 + weightOnBack(row - 1, col, pyramidHeight, table) / 2;
        }
        table[row][col] = ans;
        return ans;
    }
...
double weightOnBackOf(int row, int col, int pyramidHeight) {
     ...
     * return 0;
     */
    Grid<double> table(pryramidHeight, pryramidHeight);
